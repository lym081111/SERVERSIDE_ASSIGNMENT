...
<div class="main">
    ...
    <div class="page-header">
        <div>
            <h2 style="margin:0;"><?= isset($isCustom) ? 'Add Custom Club' : 'Club Join / Role Request' ?></h2>
            <div class="muted" style="margin-top:6px;">
                <?= isset($isCustom) ? 'Manually type in the details of your external or custom club.' : 'Pick an admin-created club and submit a request for approval.' ?>
            </div>
        </div>
    ...
    <?php if (!isset($isCustom) && empty($clubCatalog)): ?>
        <div class="card">
            <div class="muted">No active clubs found in the catalog. Please contact an administrator.</div>
        </div>
    <?php else: ?>
        <div class="card">
            <form method="POST" enctype="multipart/form-data">
                ...
                <div class="form-grid">
                    <?php if (isset($isCustom)): ?>
                        <div>
                            <label class="label">Custom Club Name</label>
                            <input class="input" type="text" name="clubName" placeholder="Enter custom club name" required>
                        </div>
                        <input type="hidden" name="requestType" value="join" id="requestType">
                    <?php else: ?>
                        <div>
                            ...
                            <select class="input" name="clubName" required>
                                ...
                                <?php foreach ($clubCatalog as $clubDef): ?>
                                    <option value="<?= htmlspecialchars((string) ($clubDef['clubName'] ?? ''), ENT_QUOTES, 'UTF-8') ?>">
                        ...
                        <div>
                            ...
                            <select class="input" name="requestType" id="requestType">
                                <option value="join">Join / Add Membership</option>
                                <option value="role_change">Role Change / Promotion</option>
                            </select>
                        </div>
                    <?php endif; ?>
