    ...
    <div class="split-layout">
            ...
            <table class="co-records-table">
                ...
                <tr>
                    ...
                    <td>
                        ...
                        <?php else: ?>
                            <a class="link" href="index.php?url=achievement/edit&id=<?= htmlspecialchars($a['achievementID'], ENT_QUOTES, 'UTF-8') ?>&search=<?= urlencode($_GET['search'] ?? '') ?>&sort=<?= urlencode($_GET['sort'] ?? '') ?>&status=<?= urlencode($_GET['status'] ?? '') ?>">Edit</a>
                            ...
                            <form method="POST" action="index.php?url=achievement/delete" style="display:inline;">
                                ...
                                <input type="hidden" name="_filter_search" value="<?= htmlspecialchars($_GET['search'] ?? '', ENT_QUOTES, 'UTF-8') ?>">
                                <input type="hidden" name="_filter_sort"   value="<?= htmlspecialchars($_GET['sort']   ?? '', ENT_QUOTES, 'UTF-8') ?>">
                                <input type="hidden" name="_filter_status" value="<?= htmlspecialchars($_GET['status'] ?? '', ENT_QUOTES, 'UTF-8') ?>">
        ...
        <div class="side-stack">
            <div class="card" style="background: linear-gradient(135deg, #1e40af 0%, #3b82f6 100%); color: #fff; border: none;">
                ...
                <div style="margin-top: 15px; display: flex; justify-content: flex-end; align-items: center; border-top: 1px solid #e5e7eb; padding-top: 10px;">
                    <a class="btn btn-secondary" style="font-size: 0.85rem; display: inline-flex; align-items: center; gap: 5px;" href="index.php?url=achievement/summary">
