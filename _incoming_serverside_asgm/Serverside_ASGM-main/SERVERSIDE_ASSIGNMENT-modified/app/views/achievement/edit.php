...
<?php
    ...
    // Preserve filter params so we can return to the same filtered page
    $filterSearch = htmlspecialchars($_GET['search'] ?? '', ENT_QUOTES, 'UTF-8');
    $filterSort   = htmlspecialchars($_GET['sort']   ?? '', ENT_QUOTES, 'UTF-8');
...
<div class="main">
    ...
    <div class="card">
        <form method="POST" enctype="multipart/form-data" class="form">
            ...
            <!-- Hidden filter fields to preserve search state after save -->
            ...
            <div class="form-grid">
                ...
                <div>
                    ...
                    <?php $currentCategory = (string) ($achievement['category'] ?? ''); ?>
                    <select class="input" name="category">
                        <option value="">Select category</option>
                        <?php foreach (['Leadership', 'Volunteerism', 'Academic', 'Technical', 'Sports', 'Community'] as $cat): ?>
                            <option value="<?= htmlspecialchars($cat, ENT_QUOTES, 'UTF-8') ?>" <?= $currentCategory === $cat ? 'selected' : '' ?>>
