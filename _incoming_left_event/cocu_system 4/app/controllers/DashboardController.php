...
class DashboardController {
    ...
    public function index() {
        ...
        $stmt = $db->prepare("SELECT COUNT(*) FROM events WHERE userID = ? AND status = 'approved'");
        $stmt->execute([$userID]);
        $eventCount = (int) ($stmt->fetchColumn() ?? 0);

        $stmt = $db->prepare("SELECT COUNT(*) FROM events WHERE userID = ? AND status = 'pending'");
        $stmt->execute([$userID]);
        $pendingEventCount = (int) ($stmt->fetchColumn() ?? 0);

        $stmt = $db->prepare("SELECT MAX(eventDate) FROM events WHERE userID = ? AND status = 'approved'");
        $stmt->execute([$userID]);
        $latestEventDate = $stmt->fetchColumn() ?: null;

        // Clubs - approved only, same club name counts as 1
        ...
        $stmt = $db->prepare("SELECT COUNT(DISTINCT clubName) FROM clubs WHERE userID = ? AND status = 'approved' AND (endDate IS NULL OR endDate = '')");
        $stmt->execute([$userID]);
        $activeClubCount = (int) ($stmt->fetchColumn() ?? 0);

        $stmt = $db->prepare("SELECT MAX(startDate) FROM clubs WHERE userID = ? AND status = 'approved'");
        $stmt->execute([$userID]);
        ...
        // Achievements - approved only
        $stmt = $db->prepare("SELECT COUNT(*) FROM achievements WHERE userID = ? AND status = 'approved'");
        $stmt->execute([$userID]);
        $achievementCount = (int) ($stmt->fetchColumn() ?? 0);
        ...
        $stmt = $db->prepare("SELECT MAX(dateReceived) FROM achievements WHERE userID = ? AND status = 'approved'");
        $stmt->execute([$userID]);
    ...
    public function transcript() {
        ...
        $stmt = $db->prepare("SELECT COUNT(*) FROM merits WHERE userID = ?");
        $stmt->execute([$userID]);
        $meritCount = (int) ($stmt->fetchColumn() ?? 0);
